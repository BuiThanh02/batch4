...
public class CategoryService {
    ...
    public BaseResponse createCategory(CategoryRequest request) throws ApiException {
        ...
        if (!request.checkIsRoot()) {
            ...
            if (parent.getCategories().contains(category)) {
                throw new ApiException(ERROR.INVALID_PARAM, "Already exist!");
            }
    ...
    public SearchResponse findAll(SearchBody searchBody) throws ApiException {
        ...
        query.append("SELECT * FROM category c WHERE 1 = 1");
        if (searchBody.getName() != null && searchBody.getName().length() > 0) {
            query.append("AND c.name LIKE '%").append(searchBody.getName()).append("%' ");
        }
        if (searchBody.getId() != null && searchBody.getId() > 0) {
            query.append("AND c.id = ").append(searchBody.getId());
        }
        query.append(" ORDER BY c.created_at ").append(searchBody.getSort());
        ...
        item.setItems(categoryRepository.findAllWithPage(String.valueOf(query), searchBody.getPage() - 1, searchBody.getLimit()));
